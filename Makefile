...
ipq40xx:
	@cd $(BUILD_TOPDIR)/uboot-1.0/ && $(MAKECMD) ipq40xx_cdp_config
	@cd $(BUILD_TOPDIR)/uboot-1.0/ && $(MAKECMD) ENDIANNESS=-EB V=1 all
	@cp $(BUILD_TOPDIR)/uboot-1.0/u-boot.bin $(BUILD_TOPDIR)/bin/temp.bin
	@make show_size
...
clean:
	@cd $(BUILD_TOPDIR)/uboot-1.0/ && $(MAKECMD) distclean
	@rm -f $(BUILD_TOPDIR)/uboot-1.0/httpd/fsdata.c
